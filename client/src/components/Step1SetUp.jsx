...
function Step1SetUp({ onStart }) {
    ...
    const handleUploadResume = async () => {
        ...
        try {
            ...
            console.log(error)
            const message = error?.response?.data?.message || "Resume analysis failed. Please try again."
            setErrorMsg(message)
            setAnalyzing(false);
        }
    }
    ...
    const handleStart = async () => {
        ...
        if (!safeRole || !safeExperience) {
            setErrorMsg("Please enter role and experience.")
            return
        }
        if (!userData) {
            setErrorMsg("Please login again and retry.")
            return
        }
        if ((userData?.credits || 0) < 50) {
            setErrorMsg("Not enough credits. Minimum 50 credits required.")
            return
        }
